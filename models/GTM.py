...
class GTrendEmbedder(nn.Module):
    def __init__(self, forecast_horizon, embedding_dim, use_mask, trend_len, num_trends, gpu_num):
        ...
        self.pos_embedding = PositionalEncoding(embedding_dim, max_len=trend_len)  ##TODO: Do we need `positional embedding`?
...
class TextEmbedder(nn.Module):
    def __init__(self, embedding_dim, cat_dict, col_dict, fab_dict, gpu_num):
        ...
        self.word_embedder = pipeline('feature-extraction', model='bert-base-uncased')  # Compore with simple embedding from v2 code
...
class GTM(pl.LightningModule):
    def __init__(self, embedding_dim, hidden_dim, output_dim, num_heads, num_layers, use_text, use_img, \
        ...
        if self.autoregressive: self.pos_encoder = PositionalEncoding(hidden_dim, max_len=self.output_len)
    ...
    def forward(self, category, color, fabric, temporal_features, gtrends, images):
        ...
        dummy_encoding = self.dummy_encoder(temporal_features)  ##TODO: Change module and name
        text_encoding = self.text_encoder(category, color, fabric)  ##TODO: input still is string
        ...
        if self.autoregressive == 1:
            # Decode
            ##TODO: pripority. Code for autoregressive
            ...
            decoder_out, attn_weights = self.decoder(tgt, memory, tgt_mask)  ##TODO: Currently not use tgt mask
    ...
    def validation_epoch_end(self, val_step_outputs):
        ...
        ##TODO: Dynamic normalization factor
